...
#### 追加 docs（検出された構成に応じて）
- `database.md`: DB スキーマ・マイグレーション方針（DB が存在する場合）
- `api.md`: エンドポイント一覧・リクエスト/レスポンス形式（API が存在する場合）
- `test.md`: テスト戦略・実行方法・カバレッジ方針（テストが存在する場合）
- `cicd.md`: CI/CD パイプラインの構成と各ステップの意味（CI 定義がある場合）
