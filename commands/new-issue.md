...
スコープを分割するかどうかは **ユーザーが決定する**。AI は判定せず、以下の 3 つの選択肢を**各々の理由付きで**ユーザーに提示し、選択を仰ぐ:

1. **1 つの issue で Phase 分割**
    - issue は 1 件に保ち、本文の Scope セクションで Phase 1 / Phase 2 / ... と段階を明示する
    - 全体は 1 つの coherent unit だが、順序依存があり段階的に進める価値がある場合に有効
    - Step 1/2 で得た事実をもとに、Phase 分割が有効と考えられる根拠（依存関係・段階的レビュー余地など）を提示する

2. **issue を分割（N 件）**
    - 独立した複数の関心ごとを別 issue として作成する
    - 分割案（各 issue のタイトル候補と役割）と分割が有効と考えられる根拠を提示する
    - 該当しそうな根拠: 独立した関心ごとの混在、ブロックしない並行作業、独立した完了条件など

3. **issue を分割しない**
    - 1 件の issue として段階分けもせずシンプルに作成する
    - スコープが十分に小さく単一の coherent unit である場合に有効

**ルール:**
- 賛否の根拠は Step 1/2 で得た**事実のみ**を引用する（推測・補完禁止）
