...
## 19. `hooks/auto-approve-readonly.sh` — 読み取り専用ツール自動承認 hook

Claude Code の PreToolUse hook として `~/.claude/settings.json` から呼び出される（matcher: `""`、全ツール対象）。
- 動作: ツール実行前に発火し、安全な読み取り専用操作を自動承認してパーミッションプロンプトを抑制する
  - **Read ツール**: 全入力を問わず常に auto-approve（`{"decision": "approve"}` を stdout に出力）
  ...
  - **複合コマンド処理**: `&&`/`||`/`;`/`|` で分割し、全セグメントが read-only ホワイトリストに一致する場合のみ approve
  - **ファイル書き込みリダイレクト検出**: `>[^&]`（`>&` 以外の `>`）を含む場合はパススルー（通常 permission フロー）
  - **その他のツール・コマンド**: 出力なしで `exit 0`（通常 permission フロー）
- 注意: `install.sh` が `hooks/auto-approve-readonly.sh` を `~/.claude/hooks/` へ symlink するが、**`settings.json` への登録は手動が必要**（`install.sh` は `settings.json` を変更しない）
- 依存: `bash`, `jq`
