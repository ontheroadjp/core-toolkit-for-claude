...
## 12. `commands/new-issue.md` — アイデアから issue 生成フロー（任意の pre-`/work` エントリポイント）
...
- Step 3: スコープ判定（**ユーザー選択必須**。AI は判定せず、Step 1/2 の事実のみを根拠に賛否を併記する）
...
### Step 3 の 3 択（スコープ判定）
1. **1 つの issue で Phase 分割**: 1 件に保ちつつ本文 Scope に Phase 1 / Phase 2 / ... を明示。順序依存・段階的レビュー余地がある場合に有効
2. **issue を分割（N 件）**: 独立した複数の関心ごとを別 issue として作成。分割案（タイトル候補・役割）と根拠を提示する
3. **issue を分割しない**: 1 件の issue として段階分けもせずシンプルに作成
