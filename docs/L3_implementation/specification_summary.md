...
PR 番号を受け取り、PR branch に checkout し、inline review comment と review body comment を取得する。コメントごとにユーザーが対応・反対返信・理由返信・skip を選び、対応する場合は実装・commit・push・返信まで行う。

根拠: `commands/review-resolve.md:1-175`

### `/codex-review` (`commands/codex-review.md`)

PR 番号を受け取り、PR ブランチに checkout し、`codex review --base <base>` に日本語 + リポジトリ固有ルールのプロンプトを渡してレビューを実行する。結果を端末に表示しながら一時ファイルに保存し、ANSI コード除去後に `gh api` で PR コメントとして投稿し、コメント URL を報告する。

根拠: `commands/codex-review.md:1-103`
...
根拠: `hooks/auto-approve-readonly.sh:73-181`
