...
- `notify-slack.sh`: Notification hook（permission prompt 等）および Stop hook（応答完了後の入力待ち）。`CLAUDE_CODE_WAIT_NOTIFY_SLACK_WEBHOOK_URL` 環境変数で指定された Slack Incoming Webhook にメッセージを POST する（未設定なら silently exit）
