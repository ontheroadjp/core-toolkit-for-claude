...
├── .gitignore                       # .DS_Store, .claude/, logs/, site/node_modules/, site/.vitepress/dist/ を除外
├── .github/
│   └── workflows/
│       └── deploy.yml               # GitHub Actions: push to main → VitePress build → GitHub Pages 自動デプロイ
...
├── site/                            # VitePress ドキュメントサイト（外部公開向け）
│   ├── package.json                 # VitePress + FontAwesome 依存
│   ├── .vitepress/
│   │   ├── config.mts               # nav・sidebar・FontAwesome CDN・テーマ設定
│   │   └── theme/                   # カスタムテーマ（ブランドカラー CSS）
│   ├── index.md                     # トップページ（hero 画像・feature cards）
│   ├── guide/                       # ユーザーガイド（installation・configuration）
│   ├── developer/                   # 開発者向け（specification_summary を include）
│   ├── concept/                     # コンセプト（L0_concept を include）
│   └── public/                      # 静的アセット（logo.svg・hero.png）
...
根拠: 2026-06-17 時点のディレクトリ実体を直接確認（`ls` コマンドで全ファイル列挙）。
...
| ドキュメントサイト | `site/` | GitHub Pages（`actions/deploy-pages` で自動デプロイ） | — |

- 根拠: `README.md:21-97`（Installation セクション）, `install.sh`（一括 symlink スクリプト）, `.github/workflows/deploy.yml`
